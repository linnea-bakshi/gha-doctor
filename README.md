...
- **Flaky tests, by name** (`--flaky-logs N`) — flaky-*job* detection tells you
  ...
  **test-report artifacts (JUnit XML / .NET TRX / NUnit3)** are consulted as a fallback (only when
...
- **Test-report artifacts as the fallback (JUnit XML / TRX / NUnit3)** (authenticated runs):
  ...
  .NET TRX files (`dotnet test --logger trx`) or NUnit3 reports
  (`nunit3-console`, `dotnet test --logger nunit`, Unity's test runner) and
