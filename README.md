...
- **Flaky tests, by name** (`--flaky-logs N`) — flaky-*job* detection tells you
  *where* it hurts; this tells you *which test*. It reads the logs of up to N
  ...
  own failure summaries — [20 framework families](https://linnea-bakshi.github.io/gha-doctor/flaky-frameworks):
  ...
  lit, and meson test. Output is ranked by how many sampled
...
- **The failing tests, by name** (authenticated runs): the job log is run
  through the same [20 framework extractors](https://linnea-bakshi.github.io/gha-doctor/flaky-frameworks)
