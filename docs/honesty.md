...
- Flaky test names (`--flaky-logs N`) come **only from failed runs whose
  ...
  failure-summary formats ([31 framework families](flaky-frameworks.md):
  ...
  swift-testing, dart/flutter test, LLVM lit, meson test, GoogleTest, CTest, doctest, bazel, cargo-nextest, node-core test.py);
