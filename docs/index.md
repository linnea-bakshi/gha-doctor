...
- **Flaky tests by name** — reads the logs of failed runs whose commit
  ...
  [31 framework families](flaky-frameworks.md) — pytest, unittest,
  go, cargo, jest, vitest, playwright, cypress, mocha, webdriverio, ava, TAP (bats/node-tap/tape/node --test), rspec, minitest,
  phpunit, exunit, maven surefire, gradle/JUnit, sbt, .NET xunit/VSTest,
  XCTest/xcodebuild, swift-testing, dart/flutter test, LLVM lit, meson, GoogleTest, doctest,
...
- **Single-run deep dives** (`--run <id|url|latest>`) — "why was this run
  ...
  named via the same 31 framework extractors as `--flaky-logs`
