...
- **Flaky tests by name** — reads the logs of failed runs whose commit
  ...
  [29 framework families](flaky-frameworks.md) — pytest, unittest,
  go, cargo, jest, vitest, playwright, cypress, mocha, webdriverio, ava, rspec, minitest,
...
- **Single-run deep dives** (`--run <id|url|latest>`) — "why was this run
  ...
  named via the same 29 framework extractors as `--flaky-logs`
