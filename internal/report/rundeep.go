...
func RunDeep(w io.Writer, s Style, d *api.RunDeep) {
	...
	if len(d.ArtifactTests) > 0 {
		fmt.Fprintf(w, "\n%s%s\n", s.bold("Failing tests — from test-report artifacts"),
			s.dim("  (JUnit XML/TRX uploaded by the run; artifacts can't be pinned to one job)"))
...
func RunDeepMarkdown(w io.Writer, d *api.RunDeep) {
	...
	if len(d.ArtifactTests) > 0 {
		...
		fmt.Fprintf(w, "_JUnit XML/TRX uploaded by the run; artifacts can't be pinned to one job._\n\n")
		shown := d.ArtifactTests
		if len(shown) > maxDeepFailedTestsShown {
			shown = shown[:maxDeepFailedTestsShown]
		}
